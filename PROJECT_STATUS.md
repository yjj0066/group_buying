...
│           ├── app/api/ai/
│           ├── modules/account/               # trust-profile-panel, ai-price-recommendation-panel
...
| **HOME-01** | **역할별 redirect** | `(landing)/page.tsx` (partial — 비로그인 랜딩 유지) |
...
| HOME-01 | 역할별 홈 | redirect만 — 역할별 전용 홈 UI 미완 |
| PLTF/STLM | VA custody | Medusa PG + VA stub 병존 |
| TRST-01 | 정량 신뢰 | MTRS score + DETL heuristic (full spec 미완) |
| RPT | 총대 리포트 | 스텁 페이지 |
| LGN-03/MCS | 비밀번호·고객센터 | UI 스텁 |
| SGN-02/03 | 가입 시 최애·역할 | preferences에서 설정 (가입 플로우 내장 미완) |

### 5.3 미구현

| ID | 항목 |
|----|------|
| OPEN-01~03 | 개봉 영상·배정 로그·전용 운영 UI |
| LGN-02, ACCT-02 | 소셜 로그인, 실명 인증 |
| APLY/CHKO/DONE | 3화면 완전 분리 (legacy checkout 병존) |
...
| CHKO-03 서버 | 클라 5분만 | `POST /hold` seat lock API |
...
| HOME-01 UI | redirect만 | 역할별 dedicated home layout |
...
| E2E 테스트 | unit 9개 | join→deposit→review→trust flow |
...
| RPT/MCS/LGN | 스텁 | 실 API |
| README/CODE_ANALYSIS | **2026-07-15 갱신** | — |
| CI/CD | turbo scripts | GitHub Actions |
| 배포 | DEPLOYMENT.md | Vercel + Supabase |

---

## 8. 주요 파일 인덱스 (2026-07-15)
...
*본 문서는 `group-buying-site` 코드베이스 실제 파일·API·워크플로우를 기준으로 작성되었습니다. 코드 레벨 분석은 [CODE_ANALYSIS.md](./CODE_ANALYSIS.md), 사용자-facing 요약은 [README.md](./README.md)를 참고하세요.*
