"use client"

import { Dialog, Transition } from "@headlessui/react"
import { Fragment } from "react"

import { BbButton } from "@modules/design-system"

type ProfileWithdrawalDialogProps = {
  open: boolean
  onClose: () => void
  onConfirm: () => void
  isSubmitting?: boolean
  labels: {
    title: string
    message: string
    confirm: string
    cancel: string
  }
}

const ProfileWithdrawalDialog = ({
  open,
  onClose,
  onConfirm,
  isSubmitting = false,
  labels,
}: ProfileWithdrawalDialogProps) => {
  return (
    <Transition appear show={open} as={Fragment}>
      <Dialog as="div" className="relative z-[80]" onClose={onClose}>
        <Transition.Child
          as={Fragment}
          enter="ease-out duration-200"
          enterFrom="opacity-0"
          enterTo="opacity-100"
          leave="ease-in duration-150"
          leaveFrom="opacity-100"
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-black/40" />
        </Transition.Child>

        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-200"
              enterFrom="opacity-0 scale-95"
              enterTo="opacity-100 scale-100"
              leave="ease-in duration-150"
              leaveFrom="opacity-100 scale-100"
              leaveTo="opacity-0 scale-95"
            >
              <Dialog.Panel className="w-full max-w-sm rounded-2xl border border-[var(--bb-line)] bg-white p-6 shadow-xl">
                <Dialog.Title className="text-base font-black text-[var(--bb-ink)]">
                  {labels.title}
                </Dialog.Title>
                <p className="mt-3 text-sm leading-relaxed text-[var(--bb-mute)]">
                  {labels.message}
                </p>

                <div className="mt-6 flex flex-col gap-2">
                  <BbButton
                    variant="cta"
                    fullWidth
                    isLoading={isSubmitting}
                    onClick={onConfirm}
                  >
                    {labels.confirm}
                  </BbButton>
                  <BbButton
                    variant="secondary"
                    fullWidth
                    disabled={isSubmitting}
                    onClick={onClose}
                  >
                    {labels.cancel}
                  </BbButton>
                </div>
              </Dialog.Panel>
            </Transition.Child>
          </div>
        </div>
      </Dialog>
    </Transition>
  )
}

export default ProfileWithdrawalDialog
