...
const PUBLISHED_PRODUCT_FIELDS =
  "+description,+handle,+status,*variants.calculated_price,+variants.inventory_quantity,*variants.images,*variants.options,+thumbnail,+images,+metadata,+tags,"
...
export const listPublishedProducts = async ({
  ...
  if (countryCode) {
    region = await getRegion(normalizeCountryCode(countryCode))
  }
  ...
  const next = useCache ? { ...(await getCacheOptions("products")) } : undefined
  ...
  try {
    ...
    console.error("[products] Failed to list published products:", error)
