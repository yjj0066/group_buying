...
| 공구 상세 (레거시) | http://localhost:8000/kr/group-buying/{id} |
...
### 공구 앱 (GB App) UI — 와이어프레임 기반 프론트엔드

Medusa 스토어프론트에 **모바일 우선 공구 앱** `(gb-app)` 라우트 그룹을 추가했습니다. 참여자·총대 모드 전환, 하단 탭 바, 온보딩(회원가입 → 계좌 등록) 흐름을 포함합니다.

| 구분 | 주요 경로 | 설명 |
|------|-----------|------|
| **참여자 홈 (SRCH/HOME)** | `/kr/home` | 아이돌 칩, 검색, 탭(전체/진행중/마감), 공구 카드 |
| **공구 상세 (DETL)** | `/kr/deals/[dealId]` | 옵션 선택, 수량, 실시간 합계, 신청 CTA |
| **참여 신청 (APLY)** | `/kr/deals/[dealId]/apply` | 배송지, 환불 계좌, Daum 우편번호 |
| **입금 (CHKO)** | `/kr/deals/[dealId]/deposit` | 5분 타이머, 가상계좌 복사, 만료 리다이렉트 |
| **완료 (DONE)** | `/kr/deals/[dealId]/complete` | 요약 + 이중 CTA |
| **내 참여 (MYJN)** | `/kr/participations`, `/kr/my/participations` | 5단계 스테퍼, 주소 변경, 송장 |
| **후기 (RPTB)** | `/kr/my/participations/[participantId]/review` | 수령 확인 + 별점 후기 |
| **총대 개설** | `/kr/seller/create/*` | 기본정보 → 상품 → 판매 → 배송 → 보증금(10만원) |
| **총대 운영** | `/kr/seller/deals/[dealId]/*` | 모집, 확정, 구매증빙, 수량검증, 수동배분, 발송, 정산 |
| **마이페이지** | `/kr/my/*` | 계좌, 내 공구, 참여, 정산, 신뢰, 프로필, 알림, 고객센터 |

**라우트 정의:** `apps/storefront/src/lib/wireframe/routes.ts` (`gbAppRoutes`, 탭 설정)

**레이아웃:** `apps/storefront/src/app/[countryCode]/(gb-app)/layout.tsx` — `GbWebNav`, `GbWebShell`, `GbAppTabBar`, `GroupBuyingModeProvider`

> `/kr/home` 은 로그인·온보딩(관심 아이돌) 완료 후 이용 가능합니다. 비로그인 시 `/kr/auth/login` 으로 리다이렉트됩니다.

### 버그 수정

| 증상 | 원인 | 조치 |
|------|------|------|
| `You cannot use different slug names for the same dynamic path ('id' !== 'participantId')` | `(gb-app)/my/participations/` 아래 **빈 `[id]` 폴더**가 `[participantId]` 와 공존 | stale `[id]/` 디렉터리 삭제 |
| 메인 페이지 **하얀 화면 / Internal Server Error** | ① slug 충돌로 dev 서버 크래시 ② `leader-settlement-view` import 경로 오류 ③ **수정 전 dev 서버를 재시작하지 않음** | import를 `../leader-settlement/...` 로 수정, dev 서버 재시작 |
| `leader-settlement-view` 빌드 실패 | `./bank-account-edit-modal` 등 잘못된 상대 경로 | `../leader-settlement/` 하위 모듈로 경로 수정 |

### 프론트엔드 성능 최적화 (INP 개선)

| 항목 | 내용 | 파일 |
|------|------|------|
| **검색 debounce (200ms)** | 키 입력마다 전체 카드 필터·리렌더 방지 | `use-debounced-value.ts`, `participant-home-browse`, `search-browse`, `my-support-view` |
| **카드 memo** | `GroupDealCard`, `BbGroupBuyCard` + `GroupDealCardList` 분리 | `group-deal-card/`, `bb-group-buy-card.tsx`, `group-deal-card-list/` |
| **카탈로그 분리** | 필터 입력과 결과 목록 memo 분리 | `group-deals-catalog/` (`CatalogResults`) |
| **모드 전환 optimistic UI** | 참여자/총대 토글 시 서버 액션 완료까지 UI 대기 제거 | `group-buying-mode-provider/` |

> Performance 탭에서 `tiptap.ProseMirror.ui-prompt-input-editor__input` 지연이 보이면 **Cursor IDE 채팅 입력창** 이슈이며, storefront 코드와는 무관합니다.

### 백엔드 미연동 · 알려진 갭 (프론트 mock/stub)

| API / 기능 | 상태 |
|------------|------|
| `POST /store/group-deals/:id/apply` | 프론트 mock fallback |
| 서버 사이드 seat hold API | 미구현 (클라이언트 5분 홀드만) |
| apply POST `quantity` 필드 | 백엔드 미반영 |
| Upstage OCR (구매증빙/송장) | `group-deal-document-ai.ts` → Flask/HYBRID stub, 프론트 연동 예정 |
...
| **GB App INP (2026-07-18)** | 검색 debounce 200ms, `React.memo` 카드, 카탈로그 결과 분리, 모드 전환 optimistic UI |
...
| 기능 | 설명 |
|------|------|
| **공구 앱 (GB App)** | `(gb-app)` — 참여자/총대 모드, 하단 탭, DETL→APLY→CHKO→DONE, 총대 10단계 운영 UI |
...
| **총대 대시보드** | hosted/[id] 또는 `/seller/deals/[dealId]` — 충원율·공석·AI 단가 추천 |
| **상품 → 공구 UI** | 연결 공구 있으면 redirect, 없으면 DETL 미리보기 |
| **마이페이지** | `/account/*` 레거시 + `/my/*` 공구 앱 마이 허브 |
...
│           │   ├── (gb-app)/            # 공구 앱 (홈·딜·총대·마이) + layout + tab bar
│           │   └── (main)/
│           │       ├── store/           # Flask 검색 결과
│           │       ├── group-buying/    # 레거시 공구 목록·상세·deposit
│           │       ├── products/[handle]/
│           │       ├── order/[id]/confirmed/
...
│           ├── modules/
│           │   ├── landing/
│           │   ├── group-buying/        # timeline, deposit, join, gb-app views, design-system cards
│           │   ├── design-system/       # BbGroupBuyCard, BbSearchInput, BbTabs 등
...
│           │   ├── layout/                # gb-app-tab-bar, gb-web-nav, gb-web-shell
│           │   └── order/                 # order-completed + behavior logger
│           └── lib/
│               ├── wireframe/routes.ts  # gbAppRoutes, 탭·모드 라우팅
...
| http://localhost:8000/kr/group-buying | 공구 목록·필터 (메인 쇼핑) |
| http://localhost:8000/kr/home | 공구 앱 홈 (로그인·온보딩 필요) |
| http://localhost:8000/kr/deals/{dealId} | 공구 앱 상세 (DETL) |
| http://localhost:8000/kr/seller/deals/{dealId} | 총대 대시보드 |
| http://localhost:8000/kr/my | 공구 앱 마이페이지 |
...
### 공구 앱 `(gb-app)` — 2026-07-18

| URL | 스펙 | 설명 |
|-----|------|------|
| `/kr/splash` | — | 앱 진입 → 로그인/홈 분기 |
| `/kr/auth/login`, `/signup`, `/bank-account` | LGN/SGN | 온보딩 |
| `/kr/home` | HOME/SRCH | 참여자·총대 모드 홈 (로그인 필요) |
| `/kr/search`, `/search/empty` | SRCH | 검색·빈 결과 |
| `/kr/deals/[dealId]` | DETL | 공구 상세 |
| `/kr/deals/[dealId]/apply` | APLY | 참여 신청 |
| `/kr/deals/[dealId]/deposit` | CHKO | 가상계좌 입금 |
| `/kr/deals/[dealId]/complete` | DONE | 신청 완료 |
| `/kr/participations/[participantId]` | MYJN | 참여 상세 |
| `/kr/my/participations/[participantId]/review` | RPTB | 수령 확인·후기 |
| `/kr/seller/create/*` | — | 총대 공구 개설 위저드 |
| `/kr/seller/deals/[dealId]/*` | DASH | 모집·구매·발송·정산 |
| `/kr/my/*` | MYP* | 마이페이지 허브 |

동적 세그먼트: participations 경로는 **`[participantId]`** 만 사용 (`[id]` 와 혼용 금지).
...
| `/kr/account/group-deals/participations/[participantId]` | MYJD | 참여 상세·후기·분쟁 |
...
| **`'id' !== 'participantId'` 라우트 오류** | `(gb-app)/my/participations/[id]/` stale 폴더 삭제 후 dev 재시작 |
| **하얀 화면 / Internal Server Error (전 페이지 500)** | slug 충돌·import 오류 수정 후 **storefront dev 서버 재시작** (`pnpm storefront:dev`), 브라우저 **Ctrl+Shift+R** |
| **`/kr/home` 이 로그인으로 이동** | 정상 — 비로그인 시 `requireCustomerForGbApp` 리다이렉트. 메인 쇼핑은 `/kr/group-buying` |
| **검색·클릭 반응 느림 (storefront)** | 2026-07-18 debounce·memo 반영 확인, dev 재시작 |
| **Cursor 채팅 입력 1~2초 지연** | IDE(tiptap) 이슈 — storefront와 무관 |
| **중첩 git repo (`medusa/` + `group-buying-site/`)** | Cursor diff `repo=<no repo>` 경고 시 **`group-buying-site` 폴더를 workspace root로 열기** |
...
| **Turbopack workspace root 경고** | `medusa/yarn.lock` 과 `group-buying-site/pnpm-lock.yaml` 공존 — `next.config`에 `turbopack.root` 설정 또는 `group-buying-site` 단독 workspace 권장 |
