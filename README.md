...
| Admin | https://group-buying-2hlq.onrender.com/app |
...
| **구매 증빙 (PURC)** | http://localhost:8000/kr/seller/deals/{dealId}/purchase-proof |
...
| 변경 | 설명 |
|------|------|
| **사이트명 변경** | **PokaCatch (포카캐치)** — 네비, 랜딩, 로그인/스플래시, 브라우저 탭 제목 등 i18n 일괄 반영 |
| **영수증 수동 입력 (PURC)** | AI 인식 실패 시 판매처·주문번호·구매일·수량·금액 직접 입력 |
| **영수증 추출값 수정** | AI 파싱 성공 후에도 **수정** 버튼으로 값 교정 → 재검증 후 저장 |
| **confirm API** | `POST /store/me/group-deals/:id/receipt/confirm` — 수동/수정 데이터 저장 + 4항목 검증 |

**영수증 수동 입력 흐름**

1. PURC 화면에서 영수증 업로드
2. AI 실패 또는 값 오류 → 수동 입력 폼 표시 (또는 **수정** 토글)
3. 필드 입력 후 **영수증 정보 저장** → `receipt/confirm` 호출
4. 4항목 검증 통과 시 `purchase_receipt_status=verified` → 다음 단계(개봉) 진행

**주요 파일:** `receipt-structured-entry-form/`, `ai-verification-panel/`, `group-deal-document-ai.ts`, `receipt/confirm/route.ts`

### 문서

| 문서 | 설명 |
|------|------|
| **[docs/MODULES.md](./docs/MODULES.md)** | Upstage API 연동, BFF·Medusa·Storefront 아키텍처, PURC/SHIP 데이터 흐름 |
...
| 기능 | 설명 | 주요 파일 |
|------|------|-----------|
| **영수증 AI (PURC)** | 구매 증빙 업로드 → BFF/Upstage 파싱 → 4항목 자동 검증 | `receipt/parse`, `group-deal-document-ai.ts` |
...
**Upstage 호출 경로:** Storefront → Medusa → **BFF만** Upstage 직접 호출 (키는 BFF에만 설정)
...
| `Document AI BFF is required` | Medusa `DOCUMENT_AI_ENABLED=true`, `HYBRID_API_URL`, `HYBRID_API_SHARED_SECRET` 설정 |
...
| 단계 | 경로 |
|------|------|
| 구매 증빙 (AI + 수동) | `/seller/deals/{id}/purchase-proof` |
...
| **Document AI** | 영수증·송장 OCR (Upstage BFF/스텁), 수동 입력·수정, 매칭 테이블 |
...
| **6개국어 UI** | ko / en / es / ru / zh / ja — 브랜드명 ko: 포카캐치, en: PokaCatch |
| **와이어프레임 라우트** | `lib/wireframe/routes.ts` — 화면 ID (PURC, SHIP, STLM, RPTG 등) |
...
├── CODE_ANALYSIS.md
├── docs/
│   ├── MODULES.md                       # Upstage·BFF·PURC/SHIP 아키텍처
│   └── upstage-receipt-integration.md
├── apps/
│   ├── backend/                         # @dtc/backend
│   │   └── src/
│   │       ├── modules/group-buying/
│   │       ├── api/store/me/group-deals/  # receipt/parse, receipt/confirm, tracking/parse, shipping/complete
...
└── services/
    └── document-ai-bff/                 # Flask BFF — Upstage API 키 보관
```
...
1. **PURC** 화면에서 영수증 캡처 업로드
2. AI 파싱 결과 확인 — 오류 시 **수동 입력** 또는 **수정** 후 저장
3. 4항목 검증 통과 (주문번호, 판매처, 구매일, 수량) 또는 Admin **Verify Receipt**

### SHIP (송장·발송)

1. **Admin**에서 해당 공구 **Verify Receipt** 완료 (또는 PURC 자동 검증 통과)
2. `apps/backend` + `apps/storefront` 실행
3. AI 사용 시: BFF 실행, `DOCUMENT_AI_ENABLED=true`, BFF `UPSTAGE_API_KEY` 설정
...
| POST | `/store/me/group-deals/:id/receipt/confirm` | 영수증 수동 입력·수정 저장 + 검증 |
...
**`receipt/confirm` 요청 본문 예시:**

```json
{
  "seller": "Weverse Shop",
  "order_number": "ORD-20260723-001",
  "ordered_at": "2026-07-22T14:30:00.000Z",
  "album_quantity": 10,
  "total_amount": 150000
}
```
...
상세: **[docs/MODULES.md](./docs/MODULES.md)** · **[CODE_ANALYSIS.md](./CODE_ANALYSIS.md)** · **[docs/api-contract-for-merge.md](./docs/api-contract-for-merge.md)**
...
| **`Document AI BFF is required`** | Medusa `DOCUMENT_AI_ENABLED`, `HYBRID_API_URL`, `HYBRID_API_SHARED_SECRET` 설정 |
| **영수증 AI 실패** | PURC 화면 **수동 입력** 사용. 이미지 업로드 후 `receipt/confirm`으로 저장 |
| **송장 업로드 실패** | BFF 실행, `DOCUMENT_AI_ENABLED`, `HYBRID_API_SHARED_SECRET` 일치 확인 |
